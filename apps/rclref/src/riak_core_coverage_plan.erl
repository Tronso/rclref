...
-spec create_plan(all | allup, pos_integer(), pos_integer(), req_id(), atom()) ->
...
create_plan(VNodeSelector, NVal, PVC, ReqId, Service) ->
    ...
    DownVNodes =
        [Index || {Index, _Node} <- offline_owners(Service, CHBin, NonCoverageNodes)],
...
n_keyspaces(VNode, N, PartitionCount) ->
    ordsets:from_list([X rem PartitionCount
                       || X <- lists:seq(PartitionCount + VNode - N, PartitionCount + VNode - 1)]).
...
next_vnode(KeySpace, Available) ->
    CoverCount =
        [{covers(KeySpace, CoversKeys), VNode, TieBreaker}
...
compare_next_vnode({CA, _VA, TBA}, {CB, _VB, TBB}) ->
    if CA > CB -> %% Descending sort on coverage
           true;
       CA < CB ->
           false;
       true ->
