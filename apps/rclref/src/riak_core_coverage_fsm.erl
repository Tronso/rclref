...
-compile({nowarn_deprecated_function,
          [{gen_fsm, start_link, 3}, {gen_fsm, start_timer, 2}]}).
...
-export([init/1, initialize/2, waiting_results/2, handle_event/3, handle_sync_event/4,
...
-callback process_results(Results :: any(), ModState :: tuple()) ->
                             ...
                             {error, Error :: any()}.
-callback process_results(Vnode :: from(), Results :: any(), ModState :: tuple()) ->
                             {ok, UpdModState :: tuple()} |
                             {done, UpModState :: tuple()} |
                             {error, Error :: any()}.
...
-spec start_link(module(), from(), [term()]) -> {ok, pid()} | ignore | {error, term()}.
...
test_link(Mod, From, RequestArgs, _Options, StateProps) ->
    ...
    gen_fsm_compat:start_link(?MODULE,
                              {test, [Mod, From, RequestArgs, Timeout], StateProps},
                              []).
...
%% @private
init([Mod, From = {_, ReqId, _}, RequestArgs]) ->
...
waiting_results({{ReqId, VNode}, Results},
                StateData =
                    #state{coverage_vnodes = CoverageVNodes,
                           mod = Mod,
                           mod_state = ModState,
                           req_id = ReqId,
                           timeout = Timeout,
